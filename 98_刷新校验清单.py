#!/usr/bin/env python3
"""刷新 校验清单.json：按CRLF转LF后的内容重算所有 markdown 的字节数与 SHA-256，
新增文件自动登记，已删除文件自动移除。改完文件后在仓库根目录跑一次：

    python 98_刷新校验清单.py

只更新 校验清单.json 的 files 和 hash_normalization 字段，不改 layout、
...
# 仅归一化CRLF；不忽略空白、BOM或正文变化。bytes与sha256使用同一字节序列。
data["hash_normalization"] = "crlf-to-lf-v1"
...
for e in entries:
    blob = open(e["path"], "rb").read().replace(b"\r\n", b"\n")
    e["bytes"] = len(blob)
